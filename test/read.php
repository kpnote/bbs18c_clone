...
#　リクエスト解析
#==================================================
if ($_SERVER['REQUEST_METHOD'] != 'GET') DispError('そんな板orスレッドないです。1');
# 2017/12/12 mod
...
if (!empty($_SERVER['SCRIPT_NAME'])) {
	$pairs = explode('/',$_SERVER['SCRIPT_NAME']);
	$bbs = $pairs[4];
	$key = $pairs[5];
...
<input type="submit" value="書き込む" name="submit">1
...
<input type="submit" value="書き込む" name="submit">2
名前： <input name="FROM" size="19">
E-mail<font size="1"> (省略可) </font>: <input name="mail" size="19"><br>
<textarea rows="5" cols="70" wrap="off" name="MESSAGE"></textarea><br>
<input type="hidden" name="bbs" value="<?=$bbs?>">
